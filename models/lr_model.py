...
def get_default_param():
    return {"solver": "newton-cg", 'C': 1e-06, 'penalty': 'l2'}

def run_grid_search(grid, data_loader):
    X, y = data_loader.get_dataset()

    tuning = GridSearchCV(
        estimator=LogisticRegression(n_jobs=cpu_count(), solver="newton-cg", max_iter=500),
        param_grid=grid,
        scoring='accuracy',
        cv=5)
    tuning.fit(X, y)
...
if __name__ == "__main__":
    param_grid = {
        'penalty': ['l2'],
        'C': [0.0000001, 0.000001, 0.000005, 0.00001, 0.0005, 0.0001]
    }
